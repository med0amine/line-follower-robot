
int sensor1 = 8;      
int sensor2 = 9;
int sensor3 = 10;
int sensor4 = 11;      


int sensor[4] = {0, 0, 0, 0};


int ENA = 3;
int motorInput1 = 7;
int motorInput2 = 6;
int motorInput3 = 5;
int motorInput4 = 4;
int ENB = 2;


int initial_motor_speed = 140;


int ledPin1 = A1;
int ledPin2 = A0;


float Kp = 25;
float Ki = 0;
float Kd = 15;

float error = 0, P = 0, I = 0, D = 0, PID_value = 0;
float previous_error = 0, previous_I = 0;

int flag = 0;

void setup()
{
  pinMode(sensor1, INPUT);
  pinMode(sensor2, INPUT);
  pinMode(sensor3, INPUT);
  pinMode(sensor4, INPUT);

  pinMode(motorInput1, OUTPUT);
  pinMode(motorInput2, OUTPUT);
  pinMode(motorInput3, OUTPUT);
  pinMode(motorInput4, OUTPUT);
  pinMode(ENA, OUTPUT);
  pinMode(ENB, OUTPUT);

  pinMode(ledPin1, OUTPUT);
  pinMode(ledPin2, OUTPUT);

  digitalWrite(ledPin1, LOW);
  digitalWrite(ledPin2, LOW);

  Serial.begin(9600);
  delay(500);
  Serial.println("Started !!");
  delay(1000);
}
void loop()
{
  read_sensor_values();
  Serial.print(error);
  if (error == 100) {               
    //Serial.print("\t");
    //Serial.println("left");
    do {
      read_sensor_values();
      analogWrite(ENA, 110); 
      analogWrite(ENB, 90); 
      sharp_left();
    } while (error != 0);

  } else if (error == 101) {       
    //Serial.print("\t");
    //Serial.println("right");
    analogWrite(ENA, 110); 
    analogWrite(ENB, 90); 
    forward();
    delay(200);
    stop();
    read_sensor_values();
    if (error == 102) {
      do {
        analogWrite(ENA, 110); 
        analogWrite(ENB, 90); 
        sharp_right();
        read_sensor_values();
      } while (error != 0);
    }
  } else if (error == 102) {        
    //Serial.print("\t");
    //Serial.println("sharp right");
    do {
      analogWrite(ENA, 110); 
      analogWrite(ENB, 90); 
      sharp_left();
      read_sensor_values();
      if (error == 0) {
        stop();
        delay(200);
      }
    } while (error != 0);
  } else if (error == 103) {     
    if (flag == 0) {
      analogWrite(ENA, 110); 
      analogWrite(ENB, 90); 
      forward();
      delay(200);
      stop();
      read_sensor_values();
      if (error == 103) {     
        stop();
        digitalWrite(ledPin1, HIGH);
        digitalWrite(ledPin2, HIGH);
        flag = 1;
      } else {        
        analogWrite(ENA, 110); 
        analogWrite(ENB, 90); 
        sharp_left();
        delay(200);
        do {
          //Serial.print("\t");
          //Serial.println("sharp left");
          read_sensor_values();
          analogWrite(ENA, 110); 
          analogWrite(ENB, 90); 
          sharp_left();
        } while (error != 0);
      }
    }
  } else {
    calcul_pid();
    motor_control();
  }
}

void read_sensor_values()
{
  sensor[0] = !digitalRead(sensor1);
  sensor[1] = !digitalRead(sensor2);
  sensor[2] = !digitalRead(sensor3);
  sensor[3] = !digitalRead(sensor4);

  /*
    Serial.print(sensor[0]);
    Serial.print("\t");
    Serial.print(sensor[1]);
    Serial.print("\t");
    Serial.print(sensor[2]);
    Serial.print("\t");
    Serial.println(sensor[3]);*/

  if ((sensor[0] == 1) && (sensor[1] == 0) && (sensor[2] == 0) && (sensor[3] == 0))
    error = 3;
  else if ((sensor[0] == 1) && (sensor[1] == 1) && (sensor[2] == 0) && (sensor[3] == 0))
    error = 2;
  else if ((sensor[0] == 0) && (sensor[1] == 1) && (sensor[2] == 0) && (sensor[3] == 0))
    error = 1;
  else if ((sensor[0] == 0) && (sensor[1] == 1) && (sensor[2] == 1) && (sensor[3] == 0))
    error = 0;
  else if ((sensor[0] == 0) && (sensor[1] == 0) && (sensor[2] == 1) && (sensor[3] == 0))
    error = -1;
  else if ((sensor[0] == 0) && (sensor[1] == 0) && (sensor[2] == 1) && (sensor[3] == 1))
    error = -2;
  else if ((sensor[0] == 0) && (sensor[1] == 0) && (sensor[2] == 0) && (sensor[3] == 1))
    error = -3;
  else if ((sensor[0] == 1) && (sensor[1] == 1) && (sensor[2] == 1) && (sensor[3] == 0)) 
    error = 100;
  else if ((sensor[0] == 0) && (sensor[1] == 1) && (sensor[2] == 1) && (sensor[3] == 1)) 
    error = 101;
  else if ((sensor[0] == 0) && (sensor[1] == 0) && (sensor[2] == 0) && (sensor[3] == 0)) 
    error = 102;
  else if ((sensor[0] == 1) && (sensor[1] == 1) && (sensor[2] == 1) && (sensor[3] == 1)) 
    error = 103;
}

void calcul_pid()
{
  P = error;
  I = I + previous_I;
  D = error - previous_error;

  PID_value = (Kp * P) + (Ki * I) + (Kd * D);

  previous_I = I;
  previous_error = error;
}

void motor_control()
{
  int left_motor_velocity = initial_motor_speed - PID_value;
  int right_motor_velocity = initial_motor_speed + PID_value;

  left_motor_velocity = constrain(left_motor_velocity, 0, 255);
  right_motor_velocity = constrain(right_motor_velocity, 0, 255);

  /*Serial.print(PID_value);
    Serial.print("\t");
    Serial.print(left_motor_velocity);
    Serial.print("\t");
    Serial.println(right_motor_velocity);*/

  analogWrite(ENA, left_motor_velocity); 
  analogWrite(ENB, right_motor_velocity - 30); 

  forward();
}

void forward()
{
  digitalWrite(motorInput1, LOW);
  digitalWrite(motorInput2, HIGH);
  digitalWrite(motorInput3, LOW);
  digitalWrite(motorInput4, HIGH);
}
void right()
{
  digitalWrite(motorInput1, LOW);
  digitalWrite(motorInput2, HIGH);
  digitalWrite(motorInput3, LOW);
  digitalWrite(motorInput4, LOW);
}
void left()
{
  digitalWrite(motorInput1, LOW);
  digitalWrite(motorInput2, LOW);
  digitalWrite(motorInput3, LOW);
  digitalWrite(motorInput4, HIGH);
}
void sharp_right() {
  digitalWrite(motorInput1, LOW);
  digitalWrite(motorInput2, HIGH);
  digitalWrite(motorInput3, HIGH);
  digitalWrite(motorInput4, LOW);
}
void sharp_left() {
  digitalWrite(motorInput1, HIGH);
  digitalWrite(motorInput2, LOW);
  digitalWrite(motorInput3, LOW);
  digitalWrite(motorInput4, HIGH);
}
void stop()
{
  digitalWrite(motorInput1, LOW);
  digitalWrite(motorInput2, LOW);
  digitalWrite(motorInput3, LOW);
  digitalWrite(motorInput4, LOW);
}